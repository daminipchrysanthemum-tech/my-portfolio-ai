...
## Challenges
Connecting Azure SQL to the Node.js backend using the `tedious` package — and yes, the package is literally called tedious, which felt accurate. Getting the connection string right and handling async queries correctly took more time than expected. Once it clicked, the connection was reliable, and the queries were fast.
